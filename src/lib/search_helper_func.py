...
class PropertyPaginatorMixin(object):
  ...
  def form(self):
    ...
    form.realestate_network.choices = [(my_key, rs.name), ('ALL','Todas')]+[(str(rs_friend.get_the_other_realestate(my_key, key_only=True)), rs_friend.get_the_other_realestate(my_key, key_only=False).name) for rs_friend in friends]
