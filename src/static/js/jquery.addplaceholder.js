	...
	$.fn.addPlaceholder = function(options){
			...
			if ($.support.placeholder) 
      {
		...
		function passPlacehold(el, ph) {
			...
			var span = $('<span/>',{
				'class': 'span_pwd_container '+el.attr('class')+' '+settings['class'],	// 'inherit' class from the input field + the placeholder class
				text: ph,
				css: {
					...
					lineHeight: el.height()+'px'		// +3+'px' //needs adjustment due to extra "natural" padding of input fields
					//,paddingLeft:parseFloat(el.css('paddingLeft'))+2+'px'	//  ditto as above
				}
			}).insertAfter(el);
			
			// setup the events for the password placeholder
      span.click(function(){el.trigger('focusin');el.focus();return false;});
			el.focusin(function(){
				if (el.hasClass(settings['class'])) {
					span.hide();
					el.removeClass(settings['class']);
